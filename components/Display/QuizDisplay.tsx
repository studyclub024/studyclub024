  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-20">
      <div className="flex items-center justify-between border-b border-gray-100 pb-6">
        <div>
          <h2 className="text-3xl font-black text-gray-950 tracking-tight">Active Assessment</h2>
          <p className="text-gray-500 font-medium text-sm mt-1">Test your knowledge of the analyzed material.</p>
        </div>
        {showResults && (
          <div className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-indigo-100 animate-scale-in">
            ...
            <div key={idx} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition-shadow">
              <div className="p-8 md:p-10">
                <div className="flex gap-5 mb-8">
                  <span className="flex-shrink-0 w-10 h-10 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center font-black text-lg">
                    {idx + 1}
                  </span>
                  <div className="text-xl text-gray-950 font-semibold pt-1 leading-relaxed w-full">
                  ...
                  {question.options.map((option, optIdx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative p-6 rounded-2xl border-2 text-left transition-all min-h-[5rem] flex items-center group/opt ${buttonStyle}`}
                      >
                        <div className="flex items-center justify-between w-full gap-4">
                          <span className="w-full text-base md:text-lg"><LatexRenderer inline>{option}</LatexRenderer></span>
      ...
      {short && short.length > 0 && (
        <div className="mt-16 space-y-8">
            <div className="flex items-center gap-3">
               ...
               <h3 className="text-xl font-black text-gray-400 uppercase tracking-widest">Constructive Responses</h3>
               <div className="h-px bg-gray-200 flex-1"></div>
            </div>
            <div className="grid gap-6">
                {short.map((item, idx) => (
                    <div key={idx} className="bg-white rounded-[2rem] p-8 md:p-10 border border-gray-100 shadow-sm">
                         <div className="text-xl font-bold text-gray-950 mb-6 flex gap-4">
                         ...
                         <div className={`transition-all duration-500 ease-out overflow-hidden ${showResults ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                             <div className="bg-indigo-600 text-white p-8 rounded-[1.5rem] shadow-xl shadow-indigo-100">
                                <span className="font-black text-[10px] uppercase tracking-widest block mb-3 opacity-70">Official Answer Key</span> 
                                <div className="text-lg leading-relaxed"><LatexRenderer className="text-white">{item.answer}</LatexRenderer></div>
                             </div>
                         </div>
