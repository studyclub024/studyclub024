...
const NotesDisplay: React.FC<Props> = ({ data }) => {
  return (
    <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
      ...
      <div className="p-8 space-y-12">
        {data.sections.map((section, idx) => (
          <div key={idx} className="relative pl-6 border-l-2 border-indigo-100 dark:border-indigo-800">
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">{section.heading}</h3>
            <ul className="space-y-3">
              {section.bullets.map((bullet, bIdx) => (
                <li key={bIdx} className="text-gray-600 dark:text-gray-300 leading-relaxed flex items-start gap-2">
