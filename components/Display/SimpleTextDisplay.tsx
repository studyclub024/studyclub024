...
const SimpleTextDisplay: React.FC<Props> = ({ data, fullWidth }) => {
  const renderContent = () => {
  ...
  return (
    <div className={`${fullWidth ? 'w-full' : 'max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-slate-700'}`}>
      {renderContent()}
    </div>
  );
};
