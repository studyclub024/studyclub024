...
import { 
  ...
  Trash2, RotateCcw, HelpCircle, FileText, List, Baby, ScanEye, Calculator, GraduationCap, Lock
} from 'lucide-react';
...
const ProfileView: React.FC<Props> = ({ profile, history, onUpdatePreferences, onLogout, onOpenUpgrade, onClearHistory, onRestoreSession, canUseThemes }) => {
  ...
  const isTopTier = profile.subscriptionPlanId === 'master';
         ...
         <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto mt-4 md:mt-0">
              ...
              <button 
                ...
                disabled={isDeleting}
                className="px-6 py-2 text-gray-400 dark:text-slate-500 font-bold text-[10px] uppercase tracking-widest hover:text-red-500 transition-colors disabled:opacity-50"
              >
         ...
         <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 border border-gray-100 dark:border-white/5 shadow-sm">
               ...
               <div className="space-y-10">
                  <div>
                     <label className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-4 ml-1">Default Learning Framework</label>
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                     <div>
                        <label className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-4 ml-1">Native Language (Tutor)</label>
                     ...
                     <div>
                        <label className="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest block mb-4 ml-1">Difficulty Baseline</label>
               ...
               <div className="space-y-3 flex-1 overflow-y-auto pr-2 scrollbar-thin">
                  {history.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-20 opacity-30 text-center">
                          ...
                          <p className="font-bold dark:text-slate-500">No sessions recorded yet.</p>
                      </div>
                  ) : (
                      history.map(item => {
...
const StatsCard = ({ icon: Icon, label, value, color, bg }: { icon: any, label: string, value: any, color: string, bg: string }) => (
  <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-8 border border-gray-100 dark:border-white/5 shadow-sm flex flex-col items-center text-center group hover:-translate-y-1 transition-all">
     ...
     <div className="text-[8px] md:text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest leading-tight">{label}</div>
  </div>
);
