...
const Leaderboard: React.FC<Props> = ({ user, onAchievement }) => {
  ...
  useEffect(() => {
    ...
    const q = usersRef
      ...
      .limit(500); // Massive limit to ensure rank stability
    ...
    const unsubscribe = q.onSnapshot((snapshot) => {
      ...
      const sortedUsers = users.sort((a, b) => {
        const genA = a.stats.dailyGenerations || 0;
        const genB = b.stats.dailyGenerations || 0;
        if (genA !== genB) return genB - genA;

        const dateA = a.stats.lastActiveDate || 0;
        const dateB = b.stats.lastActiveDate || 0;
        if (dateA !== dateB) return dateB - dateA;

        return (a.userId || '').localeCompare(b.userId || '');
      });

      // Optimistic transition: Only stop loading when we actually have data
      ...
      if (user) {
        ...
        if (currentRank !== null) {
          ...
          if (currentRank === 1) activeAchievement = { ...commonFields, type: 'champion' };
          else if (currentRank <= 3) activeAchievement = { ...commonFields, type: 'elite' };
          else if (currentRank <= 10) activeAchievement = { ...commonFields, type: 'pro' };
          else if (isSignificantJump) activeAchievement = { ...commonFields, type: 'leap', fromRank: prevBest };
          else if (currentRank <= 100) activeAchievement = { ...commonFields, type: 'rank' as any };

          if (prevBest === null || currentRank < prevBest) {
            bestRankSessionRef.current = currentRank;
          }
        }
        onAchievementRef.current(activeAchievement);
      }
    },
      (error: any) => {
        if (!isMounted) return;
  ...
  return (
    <div className="w-full bg-white/30 dark:bg-black/20 backdrop-blur-md border-y theme-border py-3 overflow-hidden relative group flex items-center transition-all duration-700">
      ...
      <div className="flex items-center gap-3 px-6 shrink-0 z-10 relative">
        <div className="flex items-center gap-2 py-1 px-3 theme-bg rounded-full shadow-lg theme-glow">
          <Zap size={14} className="fill-current" />
          <span className="text-[10px] font-black uppercase tracking-[0.1em]">Top 10 Leaderboard</span>
        </div>
      </div>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[9px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-tighter">
